---
title: "Team Structures and Performance of Winning Kaggle Teams"
author: "Jim Thompson"
date: "August 15, 2015 (Revised: August 20, 2015)"
...
# data only covers teams placing in the top 3 positions.
...
# save original graphic parameters
orig.par <- par()
...
createGraphData <- function (raw.team.df) {
  ...
  vertices.df <- adply(vertices.df,1,mutate,
                       ...
                       size=switch(min(top3.count,3),3,7,9))
...
we examine the relationships of Kaggle players on winning teams, i.e.,placing in the top 3, using the **igraph** package.  In addition, we look the performance multi-player vs single-player teams.
The Kaggle competitions covered by this analysis are shown in the Appendix.
...
In terms of team structure we find over the course of many competitions, a person teaming up 
with one set of players for a competition teaming up with 
a different set of players on another competition.  These relationships 
eventually build up to separate communities of players.

From a performance view we found a higher percentage of multi-player teams achieving 
a Gold or Silver medal.  We hypothesize team collaboration is one factor for this 
observation.


### Team Structure
Team structures are depicted using a graph model.  In this model, Kaggle
players are represented as vertices of a graph.  An edge exists between two vertices if
the players represented by those vertices competed on the same team in a Kaggle 
competition.  We illustrate this concept with the following example.

For the Heritage Health Prize, which ended in April 2013, the top 3 teams are shown
in the following table. 

Place|Team Name|Team Members
-----|---------|-------------
1|POWERDOT|David Vogel, Sali Mali, Randy, Edward, Willem Mestrom, infty, random modeler
2|EXL Analytics|Sassoon Kosian, DanB, B Yang, Tim Salimans
3|J.A.Guerrero|J.A. Guerrero

This is the graph representation of the above table.  Vertex color indicates place of the player's team.
...
plot(g,
     layout=lyt,
     main="Heritage Health Prize")
legend(-0.6,-1.2,
       c("Gold","Silver","Bronze"),
       fill=c("gold","darkgray","darkgoldenrod1"),
       cex=1.0,
       bty="n",
       horiz=TRUE)
```

When considering all `r length(unique(raw.team.df$competition.name))` competitions
we have the following graph representation.  Because a Kaggle player may have competed 
in more than one competition on different teams, we use the color and size of the vertex to 
convey additional information.  Vertex color indicates the highest position a 
player's team placed and size indicates the number of times the player's teams
placed in the top 3.  

```{r,echo=FALSE,fig.height=7,fig.width=7}
# load up all competition data
df <- raw.team.df
graph <- createGraphData(df)
...
From this representation we can see subsets of Kaggle players forming communities.

This graph depiction allows us to calculate several measures.  First the number of 
vertices in the graph, `r length(V(g))`, indicate the number Kaggle
players that participated on teams that placed in the top 3.  For each player we
can determine the number of other players he or she teamed up with by counting the
number of edges that terminate at the vertex representing the player.  This number is 
called the vertex degree.  By counting the number of vertices with degree zero, we find 
there are `r sum(degree(g,V(g))== 0)` players out of the `r length(V(g))` that
did not participate on a team.  This represents 
`r round(100*sum(degree(g,V(g))== 0)/length(V(g)),0)`% of the players.  The histogram
shows the distribution of degree values.

```{r,echo=FALSE,fig.width=6,fig.height=4}
qplot(degree(g,V(g)),geom="histogram",binwidth=1) +
    xlab("Vertex Degree") +
    ylab("Count") +
    ggtitle("Vertex Degree Distribution")
```

We now turn our attention to look at certain relationships.  As of August 13th, 
the Top 10 Kagglers are 
...
# get top 10 player names from graph data
top.players <- adply(top.players,1,mutate,
                     member.name=V(g)[member.url]$member.name)

print(top.players[c("rank","member.name")],row.names=FALSE)

```

The Top 10 Kagglers on the graph are identified by name and a red line outlining the
player's vertex. From this we see that 7 out of the
10 top Kagglers participated on one or more teams and 3 were lone competitors.

```{r,echo=FALSE,fig.height=7,fig.width=7}

# identify the top 10 players on the graph
V(g)$shape <- "circle"
...
For the Top 10 Kagglers, we now show the players that were on the same teams.  In the
graphs below, the thickness of the line joining two vertices indicate how often
the players participated on a team.  A thin line is for 1 time, a thicker line indicates
more than one time.  For example, Gilberto Titericz Junior (Rank #2) partnered with
Stanislav Semenov one time and with Leustagos more than once.

```{r,echo=FALSE,warning=FALSE,fig.height=15,fig.width=7}

# function to focus on the neighbors of the top players
plotTopPlayerNeighbors <- function(one.row,g) {
#     cat(one.row$member.url,"\n")
#     flush.console()
  top.v <- V(g)[one.row$member.url]
  
  # check if top player has neighbors
  if (degree(g,top.v) != 0) {
      # yes- get neighboring players
       e1 <- incident_edges(g,top.v)
      ...
      # no-just get top player vertex
      g.top.player.neighbors <- induced_subgraph(g,top.v)
  }
 
  V(g.top.player.neighbors)$size <- V(g.top.player.neighbors)$size * 4
  if (degree(g.top.player.neighbors,V(g.top.player.neighbors)[one.row$member.url]) == 0 || 
      degree(g.top.player.neighbors,V(g.top.player.neighbors)[one.row$member.url]) == 4) {
      lyt <- layout_in_circle(g.top.player.neighbors)
  } else {
      lyt <- layout_nicely(g.top.player.neighbors)
  }
...
# specify layout for top players
layout(matrix(1:10,ncol=2,byrow = TRUE))
par(oma=c(0,0,3,0))
a_ply(top.players[1:10,],1,plotTopPlayerNeighbors,g)
mtext("Teammates of Top Ten Players",side = 3,line=0,outer = TRUE,font=2,cex=1.1)
layout(matrix(1))
par(orig.par)

```

From the Top Ten Players chart, we see several of the Top Ten Players, e.g., Owen Zhang, 
Gilberto Titericz Junior and others are related in a series of team relationships.  The 
graph below shows this community of players.  
...
set.seed(14)
plot(g3,
     main="Community of Kaggle Players")
legend(-0.5,-1.1,
       c("Gold","Silver","Bronze"),
       fill=c("gold","darkgray","darkgoldenrod1"),
       cex=0.75,
...
For this community of players we calculate the Google PageRank of each player. 
From this we see that Xavier Conort and
Leustagos have the top 2 PageRank values and play a pivotal role by connecting many players together in this community.

```{r,echo=FALSE}
# calculate pagerank of each player
pr <- page_rank(g3)

# order player by descending value of pagerank
page.rank <- pr$vector[order(-pr$vector)]

# determine player name
names(page.rank) <- sapply(names(page.rank),function(node){V(g3)[node]$member.name})

cat("Page Rank Measure of Kaggle Players\n")
print(page.rank)
```
...
# set up data set for comparing team vs single player performance
team.vs.single.df <- adply(raw.team.df,1,mutate,
                          medal=factor(switch(team.place,"Gold","Silver","Bronze"),
                                       levels=c("Gold","Silver","Bronze")))
team.vs.single.df$team.type <- str_extract(team.vs.single.df$team.type,
                                            "(team-player|single-player)$")
team.vs.single.df$team.type <- str_replace(team.vs.single.df$team.type,
                                             "team-player","multi-player")

# calculate medal percentage by teams vs single player
part1 <- ddply(team.vs.single.df,.(competition.name,medal,team.type),summarize,count=1)
part2 <- ddply(part1,.(team.type),summarize,overall.count=sum(count))
part3 <- ddply(part1,.(team.type,medal),summarize,count=sum(count))
tvs.plot.df <- merge(part3,part2)
tvs.plot.df <- adply(tvs.plot.df,1,mutate,percent=100*count/overall.count)
```

For this part of the analysis we take an assessment on whether a multi-player team
performs better or worse than single-player team. For the `r length(unique(team.vs.single.df$competition.name))` 
competitions covered by this analysis, we find `r nrow(unique(team.vs.single.df[c("competition.name","medal")]))`
records on teams placing in the top 3 positions.  Of the `r nrow(unique(team.vs.single.df[c("competition.name","medal")]))` records, we have
`r nrow(unique(team.vs.single.df[team.vs.single.df$team.type=="multi-player",c("competition.name","medal")]))`
multi-player records and `r nrow(unique(team.vs.single.df[team.vs.single.df$team.type=="single-player",c("competition.name","medal")]))` single-player records.  

There is a wide difference in the number of multi-player vs single-player teams.  Because of this 
we will look within each team type the percentage of teams achieving Gold, Silver or Bronze medals.

The medal distribution of the multi-player and single-player teams is shown in the chart below.
The chart shows for each team type the percentage, and in parenthesis the 
number of teams, achieving Gold, Silver or Bronze medals.

```{r,echo=FALSE}
# determine location for printing labels
find_midpoints <- function(df) {
  cumsums <- c(0, cumsum(df$percent))
  diffs <- diff(cumsums)
  pos <- head(cumsums, -1) + (0.5 * diffs)
  return(data.frame(team.type=df$team.type,
                    medal=df$medal,
                    count=df$count,
                    overall.count=df$overall.count,
                    percent=df$percent,
                    pos,
                    stringsAsFactors = FALSE))
}
tvs.plot.df <- ddply(tvs.plot.df,.(team.type),find_midpoints)

# plot chart
ggplot(tvs.plot.df) +
    # facet_grid(.~team.type) +
    geom_bar(aes(x=team.type,y=percent,fill=medal,binwidth=0.5),stat="identity") +
    scale_fill_manual(values=c("gold","darkgray","darkgoldenrod1"),
                      name="Medal") +
    geom_text(aes(x=team.type,y=pos,label=paste0(round(percent,1),"%\n(",count,")"))) +
    xlab("Team Type") +
    ylab("Percent") +
    ggtitle("Team Type vs Medal Position") +
...
From the chart we see `r paste0(round(subset(tvs.plot.df,team.type=="multi-player" & medal=="Gold",select="percent"),1),"%")`
of the multi-player teams achieved a Gold medal compared to 
`r paste0(round(subset(tvs.plot.df,team.type=="single-player" & medal=="Gold",select="percent"),1),"%")` of single-player teams.  The percentage of multi-player teams, `r paste0(round(subset(tvs.plot.df,team.type=="multi-player" & medal=="Silver",select="percent"),1),"%")`, achieving Silver was slightly 
higher than single-player teams, `r paste0(round(subset(tvs.plot.df,team.type=="single-player" & medal=="Silver",select="percent"),1),"%")`.  Only in the Bronze medal did single-player teams have a
higher percentage (`r paste0(round(subset(tvs.plot.df,team.type=="single-player" & medal=="Bronze",select="percent"),1),"%")`) than multi-player teams (`r paste0(round(subset(tvs.plot.df,team.type=="multi-player" & medal=="Bronze",select="percent"),1),"%")`).

This data suggest persons participating on multi-player teams may place higher than if they
competed as a single player.  This may be the result of the colloaboration of team members.

### Appendix
List of Kaggle competitions covered by this analysis.
```{r,echo=FALSE}
competition.list <- unique(raw.team.df$competition.name)
competition.list <- competition.list[order(competition.list)]
competition.list
