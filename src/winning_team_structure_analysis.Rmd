...
we examine the relationships of Kaggle players on winning teams, i.e.,placing in the top 3, using the **igraph** package.  The Kaggle competitions covered by
...
team.vs.single.df$player.type <- str_extract(team.vs.single.df$team.type,
                                            "(team-player|single-player)$")

# calculate medal percentage by teams vs single player
part1 <- ddply(team.vs.single.df,.(competition.name,medal,player.type),summarize,count=1)
part2 <- ddply(part1,.(player.type),summarize,overall.count=sum(count))
part3 <- ddply(part1,.(player.type,medal),summarize,count=sum(count))
team.vs.single.df <- merge(part3,part2)
team.vs.single.df <- adply(team.vs.single.df,1,mutate,percent=100*count/overall.count)
...
find_midpoints <- function(df) {
  ...
  return(data.frame(player.type=df$player.type,
                    medal=df$medal,
                    count=df$count,
...
team.vs.single.df <- ddply(team.vs.single.df,.(player.type),find_midpoints)

# plot chart
ggplot(team.vs.single.df) +
    # facet_grid(.~player.type) +
    geom_bar(aes(x=player.type,y=percent,fill=medal,binwidth=0.5),stat="identity") +
    scale_fill_manual(values=c("gold","darkgray","darkgoldenrod1"),
                      name="Medal") +
    geom_text(aes(x=player.type,y=pos,label=paste0(round(percent,1),"%\n(",count,")"))) +
    xlab("Player Type") +
    ylab("Percent") +
    ggtitle("Player Type vs Medal Position") +
